...
let lifes = 3;
document.querySelector('#life').textContent = lifes;
...
const tomouDano = new Audio('SOUNDS/dano-dragon.mp3');
...
const hitDragon = () => {
    planersBombs.forEach((bomb, bombIndex) => {
        if(bomb.positionX <= dragao.positionX + dragao.width &&
            ...
            tomouDano.play();
            tomouDano.currentTime = 0;
            planersBombs.splice(bombIndex, 1);
            document.querySelector('#life').textContent = lifes; 
            explosionPlane.push({
                explosion: new Explosion(dragao.positionX, dragao.positionY, explosionImage),
                time: Date.now()
            });
...
const zerouGame = () => {
    if(level >= 13) {
        ...
        janelaTitle.textContent = 'PARABÉNS!';
        janelaMessage.textContent = 'Você acabou com o Game, passou todos os levels!';
        janelaButton.textContent = 'VOLTAR À PÁGINA INICIAL'
        janelaButton.onclick = () => {
